...
bool convertFile(std::string fileName) {
	...
	if (fileHandle) {
		...
		fclose(fileHandle);
		if (fileSize >=0x0310 && !memcmp(&fileData[0], "HMIMIDI", 7)) {
			uint32_t  numberOfTracks =fileData.at(0x30) | fileData.at(0x31) <<8 | fileData.at(0x32) <<16 | fileData.at(0x33) <<24;
			uint32_t  timeBase       =fileData.at(0x34) | fileData.at(0x35) <<8 | fileData.at(0x36) <<16 | fileData.at(0x37) <<24;
			uint32_t  ticksPerSecond =fileData.at(0x38) | fileData.at(0x39) <<8 | fileData.at(0x3A) <<16 | fileData.at(0x3B) <<24;
			uint32_t  songDuration   =fileData.at(0x3C) | fileData.at(0x3D) <<8 | fileData.at(0x3E) <<16 | fileData.at(0x3F) <<24;
			uint8_t*  tracksPriority =&fileData[0x0040];
			uint8_t*  tracksDevices  =&fileData[0x0080];
			uint32_t  position       =0x0308;
			...
			for (int trackNumber =0; trackNumber <numberOfTracks; trackNumber++) {
				...
				uint32_t foundTrackNumber  =fileData.at(position +0) | fileData.at(position +1)<<8 | fileData.at(position +2)<<16 | fileData.at(position +3)<<24;
				uint32_t trackLength       =fileData.at(position +4) | fileData.at(position +5)<<8 | fileData.at(position +6)<<16 | fileData.at(position +7)<<24;
				uint32_t midiChannelNumber =fileData.at(position +8) | fileData.at(position +9)<<8 | fileData.at(position +10)<<16 | fileData.at(position +11)<<24;
				...
				uint8_t* trackDevices =tracksDevices +trackNumber *20;
				bool hasDeviceList =false;
				for (int i =0; i <5; i++) if (trackDevices[i *4 +1] ==0xA0) {
					hasDeviceList =true;
					uint8_t device =trackDevices[i *4 +0];
				...
				uint32_t priority =tracksPriority[midiChannelNumber *4 +0] | tracksPriority[midiChannelNumber *4 +1] <<8 | tracksPriority[midiChannelNumber *4 +2] <<16 | tracksPriority[midiChannelNumber *4 +3] <<24;
			...
			fprintf(stderr, "Not a proper HMIDIP file\n");
	} else
